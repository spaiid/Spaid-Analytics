...
function renderAll(detail: StockDetail): string {
  return renderToStaticMarkup(
    <MemoryRouter>
    <div>
      <StockHeader detail={detail} />
      <FairValueCard ticker={detail.ticker} fairValue={detail.fair_value} />
      <ScoreBreakdown ticker={detail.ticker} categories={detail.categories} composite={detail.score} coverage={detail.coverage} />
...
const noneScored: StockDetail = {
  ...
  categories: full.categories.map((category) => ({
    ...
    score: null,
    contribution: null,
    metrics: category.metrics.map((metric) => ({ ...metric, status: 'missing', contribution: null, score: null })),
  })),
};

const errorHtml = renderToStaticMarkup(
  <MemoryRouter>
    <StockLoadError ticker="ZZZZ" message="ZZZZ is not in the current universe." path="/api/stock/ZZZZ" />
  </MemoryRouter>,
);
...
const fullHtml = renderAll(full);
const noneHtml = renderAll(noneScored);
const sparseHtml = renderAll(sparse);

const checks: Array<[string, boolean]> = [
  ['full: no NaN', !fullHtml.includes('NaN')],
  ['sparse: no NaN', !sparseHtml.includes('NaN')],
  ['full: no ="undefined"', !fullHtml.includes('="undefined"')],
  ['sparse: no ="undefined"', !sparseHtml.includes('="undefined"')],
  ['full: no "undefined" text', !/>undefined</.test(fullHtml)],
  ['full: no empty aria-label', !fullHtml.includes('aria-label=""')],
  ['full: charts expose role="img"', (fullHtml.match(/role="img"/g) ?? []).length >= 3],
  ['full: meters present', fullHtml.includes('role="meter"')],
  ['full: sortable headers carry aria-sort', fullHtml.includes('aria-sort=')],
  ['full: category cards are expandable buttons', fullHtml.includes('aria-expanded="false"')],
  ['full: collapsed panel is hidden', fullHtml.includes('id=":') ? true : fullHtml.includes('hidden')],
  ['full: stale banner rendered', fullHtml.includes('Stale data')],
  ['sparse: no stale banner', !sparseHtml.includes('Stale data')],
  ['full: not-applicable distinct from missing', fullHtml.includes('missing-na') && fullHtml.includes('missing-missing')],
  ['full: thin peers rendered', fullHtml.includes('missing-thin')],
  ['full: upside signed percent', fullHtml.includes('+11.8%')],
  ['full: skipped method labelled', fullHtml.includes('Skipped')],
  ['full: caveats heading', fullHtml.includes('Why this estimate may be wrong')],
  ['full: composite arithmetic footer', fullHtml.includes('composite reported by the API') && fullHtml.includes('÷ their weights')],
  ['full: confidence label beside score', fullHtml.includes('Moderate')],
  ['full: peer self marked', fullHtml.includes('This company') && fullHtml.includes('is-self')],
  ['full: trap signal fired label', fullHtml.includes('Fired')],
  ['sparse: empty states, not error states', sparseHtml.includes('No fair value estimate for this company')],
  ['sparse: no role="alert"', !sparseHtml.includes('role="alert"')],
  ['none-scored: no NaN', !noneHtml.includes('NaN')],
  ['none-scored: no Infinity', !noneHtml.includes('Infinity')],
  ['none-scored: dropped-weight explanation', noneHtml.includes('did not have enough data to score')],
  ['none-scored: no arithmetic to show', noneHtml.includes('there is no arithmetic to show')],
  ['error: role="alert"', errorHtml.includes('role="alert"')],
  ['error: shows the API message verbatim', errorHtml.includes('ZZZZ is not in the current universe.')],
  ['error: links back to Opportunities', errorHtml.includes('href="/opportunities"')],
  ['error: is not an empty state', !errorHtml.includes('class="empty"')],
  ['loading: skeletons, not an empty state', loadingHtml.includes('skeleton') && !loadingHtml.includes('class="empty"')],
  ['loading: polite live region', loadingHtml.includes('aria-live="polite"')],
  ['loading: no NaN', !loadingHtml.includes('NaN')],
  ['no console errors or warnings', errors.length === 0],
];
