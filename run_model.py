...
def detect_target(df: pl.DataFrame, horizon: int) -> str:
    excess = f"excess_fwd_{horizon}d"
    plain = f"ret_fwd_{horizon}d"
    if excess in df.columns:
        return excess
    if plain in df.columns:
        return plain
    for c in df.columns:
        if c.startswith("excess_fwd_") or c.startswith("ret_fwd_"):
            return c
...
def time_split(df: pl.DataFrame, test_fraction: float):
    ...
    k = max(1, int(len(dates) * (1 - test_fraction)))
    split_date = dates[k]
...
def backtest_topN_tranches(
    ...
    N=3,
    H=90,                 # horizon (bars)
    K=9,                  # number of tranches; use divisors of H (e.g., 9 -> step=10 for H=90)
    tc_bps=5.0,
    max_weight=0.10,
    long_short=False,
):
    # Build K sub-portfolios, each rebalanced every step=H/K bars, offset by k*step
    ...
    for i, d in enumerate(dates):
        k = i % K  # which tranche rolls today
        # --- rebalance tranche k on its schedule ---
        ...
        if snap.height:
            ...
            if names:
                if len(names) > 0:
                    # equal-weight by default; you can reuse RANK_WEIGHTED logic here if desired
        ...
        # --- realize that day's return for tranche k only ---
        ...
        # other tranches keep their current weights; their daily ret is realized when they roll next

    # aggregate across K tranches by summing daily PnL contributions (i.e., mean return if equal AUM per tranche)
    # align lengths (tranches may differ by <= step items)
    L = max(len(x) for x in tr_daily) if tr_daily else 0
    agg = np.zeros(L, float)
    ...
    eq = (1.0 + r).cumprod()
    sharpe = (r.mean() / (r.std(ddof=1) + 1e-12)) * np.sqrt(252.0)  # now daily-ish contributions
...
def rolling_cv(df: pl.DataFrame, features: List[str], target: str, folds: int = 4) -> Dict[str, float]:
    # expanding window CV over time (train up to d_train_end, test until d_test_end)
    ...
    for frac in np.linspace(0.5, 0.9, num=folds):
        k = max(5, int(len(dates) * frac))                 # train uses first k dates
        d_train_end = dates[k - 1]
        d_test_end_idx = min(k + max(5, len(dates) // 20), len(dates) - 1)
        d_test_end = dates[d_test_end_idx]                 # <-- FIX: convert index -> date
...
if __name__ == "__main__":
    # --- choose the right features file for the requested horizon ---
    ...
    use_list = pref if pref else cands  # fallback if no tagged matches

    def end_date_key(p: Path):
        try:
            base = p.name[:-8]  # strip ".parquet"
            tail = base.split("_to_")[1]
            end_str = tail.split("_fwdH_")[0]  # handles both *_fwdH_90 and *_fwdH_20-90
            return end_str
        except Exception:
            return ""
    ...
    # pick the newest that actually contains the desired target column
    def detect_in_file(p: Path) -> bool:
        try:
            df_head = pl.read_parquet(p, n_rows=10)  # fast peek
            _ = detect_target(df_head, FWD_H)       # reuses your function
            return True
        except Exception:
            return False

    # prefer the most recent usable
    ...
    print(f"[debug] feature columns in file ({len(df.columns)}): {df.columns}")
    ...
    # Keep only features that exist; warn about missing ones once.
    present = set(df.columns)
    missing = [f for f in FEATURES if f not in present]
    FEATURES = [f for f in FEATURES if f in present]
    if missing:
        print(f"[warn] dropping {len(missing)} missing features: {missing[:12]}{' ...' if len(missing)>12 else ''}")
    if not FEATURES:
        raise SystemExit("No usable FEATURES found in features parquet. Rebuild features or adjust list.")

    # If you use CS_ZSCORE later, only zscore the FEATURES that exist
    ZSCORE_COLS = FEATURES[:]  # copy for scaling step

    # Safety: if target for FWD_H is not present (e.g., you're falling back), detect again or error out
    target = detect_target(df, FWD_H)

    # Keep rows with complete features/target
    cols_needed = ["asof", "entity"] + FEATURES + [target]
    missing = [c for c in cols_needed if c not in df.columns]
    if missing:
        raise SystemExit(f"Missing columns {missing}. Rebuild features or adjust FEATURES list.")

    df = df.select(cols_needed).drop_nulls()
    df = ensure_date(df)

    # Optional: feature scaling / target demeaning
    if CS_ZSCORE:
        df = zscore_by_date(df, ZSCORE_COLS, "asof")
    if DEMEAN_TARGET:
    ...
    metrics = metrics_from_preds(yte, pred)
    ...
    N=TOPN,
    H=FWD_H,
    K=max(3, FWD_H // 10),   # e.g., 9 for H=90
    tc_bps=TC_BPS,
    max_weight=MAX_WEIGHT,
    long_short=LONG_SHORT,
    )

    print(f"backtest top{TOPN} (tranches):", bt_tr)
    ...
    # Choose dataset for audit (test-only vs all rows)
    if LEAKAGE_ON_ALL:
        # Predict on ALL rows (train+test) for audit power only (doesn't affect evaluation)
        ...
        # also build a past-feature join if available
        df_all_for_past = df
    else:
        audit_df = dfp
        df_all_for_past = df  # still needed for past-feature lookup

    # Baseline: future Rank-IC on chosen audit_df
    ric_future = rank_ic(audit_df, "y_pred", "y_true")

    # Past-H baseline using existing feature (ret_{H}d) if present
    ...
    # H-shift misalignment checks (on audit_df)
    ...
    # Placebos (shuffle / permute within date)
    ...
    # Optional global Rank-ICs (no per-date grouping), for extra signal checks
    global_stats = {}
    if PRINT_GLOBAL_RIC:
        global_stats = {
    ...
    # --- Prints ---
    print(f"model: {MODEL}")
    print("metrics (holdout):", {"r2": metrics["r2"], "rmse": metrics["rmse"],
                                 "n_train": int(train_df.height), "n_test": int(test_df.height)})
    print("rolling_cv (expanding):", cv)
    ...
    print("signal quality:", {"IC_mean": ic_mean, "IC_std": ic_std, "RankIC_mean": ric_mean})
    try:
        print("deciles (mean_ret):", list(zip(deciles["decile"].to_list(), np.round(deciles["mean_ret"].to_numpy(), 6))) )
    except Exception:
        pass
    if feat_imp:
        print("top feature importance:", feat_imp[:10])
    print("latest ranking (top 5):", list(ranking.items())[:5])
    feats = sorted((Path(os.environ["SF_DATA_ROOT"]) / "features").glob("daily_*.parquet"))
    
    print(f"[info] using features file: {feats[-1].name} from {feats[-1].parent}")
    ...
    print("leakage_audit:", leak_summary)
